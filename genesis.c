...
static void run_dumps(genesis_context *gen)
{
	...
	{ extern FILE *trace_comm, *trace_flip, *trace_dreq; if (trace_comm) fclose(trace_comm); if (trace_flip) fclose(trace_flip); if (trace_dreq) fclose(trace_dreq); }
	{ extern uint64_t cachestat[2][4]; if (gen->mars) fprintf(stderr, "CACHESTAT master reads=%llu fetch-fills=%llu data-fills=%llu bypass=%llu | slave reads=%llu fetch-fills=%llu data-fills=%llu bypass=%llu\n", (unsigned long long)cachestat[1][0], (unsigned long long)cachestat[1][1], (unsigned long long)cachestat[1][2], (unsigned long long)cachestat[1][3], (unsigned long long)cachestat[0][0], (unsigned long long)cachestat[0][1], (unsigned long long)cachestat[0][2], (unsigned long long)cachestat[0][3]); }
...
static m68k_context *sync_components(m68k_context * context, uint32_t address)
{
	...
	{ extern uint32_t s32x_trace_frame; extern vdp_context *s32x_trace_vdp; s32x_trace_frame = v_context->frame; s32x_trace_vdp = v_context; }
